...
- Последняя проверка: `npm run compile`, `npm run bundle`, `node test-extension.js`, `scripts/run-e2e-smoke.ps1`, `scripts/capture-visual-regression.ps1`, `:app:testDebugUnitTest`, `:app:assembleRelease`.
...
- Visual regression снимается скриптом `scripts/capture-visual-regression.ps1`; PNG сохраняются в `artifacts/screenshots/`, baseline хранится в `test-fixtures/visual-baseline/`.
