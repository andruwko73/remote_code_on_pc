...
- Android APK: `1.0.99`.
- VS Code extension: `1.1.96`.
- Порт API по умолчанию: `8799`.
- Внешний доступ требует публичный URL и токен.
- Последняя проверка: `npm run compile`, `node test-extension.js`, `:app:testDebugUnitTest`, `:app:assembleRelease`, установка APK на эмуляторе, внешнее подключение через публичный URL и обновление APK `1.0.98 -> 1.0.99` через кнопку в приложении.
...
- Pairing payload поддерживается двумя способами: вставка строки `remote-code-pair:...` и сканирование QR камерой.
...
4. Создайте токен и скопируйте `Copy Android pairing payload`.
5. В приложении включите внешний режим и вставьте payload или отсканируйте QR.
