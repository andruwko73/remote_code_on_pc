...
- APK: `apk/app-debug.apk`, версия `1.0.50`.
- VSIX: `extension/remote-code-on-pc-1.1.49.vsix`, версия `1.1.49`.
- APK raw URL: https://raw.githubusercontent.com/andruwko73/remote_code_on_pc/main/apk/app-debug.apk
- VSIX raw URL: https://raw.githubusercontent.com/andruwko73/remote_code_on_pc/main/extension/remote-code-on-pc-1.1.49.vsix
...
- внешний доступ через ngrok или cloudflared с проверкой готовности `/api/status`;
...
```powershell
code --install-extension C:\Users\Admin\Documents\git\remote_code_on_pc\extension\remote-code-on-pc-1.1.49.vsix --force
```
После установки:
...
- cloudflared/Cloudflare, если найден `cloudflared.exe` или команда `cloudflared` в PATH;
- ngrok, если Cloudflare не найден и есть рабочий `ngrok.exe` или команда `ngrok` в PATH.

Поэтому публичный URL вида `https://...trycloudflare.com` - это нормальный Cloudflare-туннель, а не ошибка.

Расширение публикует URL только после того, как внешний адрес ответил на `/api/status`. Если приложение показывает `503`, `Bad Gateway` или DNS-ошибку `No address associated with hostname`, почти всегда это старый или еще не готовый одноразовый адрес Cloudflare. Запустите туннель заново в VS Code, дождитесь нового URL и вставьте именно его. Старые `trycloudflare.com` адреса могут перестать резолвиться сразу после остановки cloudflared.

Для Cloudflare используется `--protocol http2`, потому что в некоторых сетях QUIC/UDP дает `timeout: no recent network activity`, `502` или `530`, хотя сам URL уже выдан.

В приложении:

1. На стартовом экране или в настройках включите `Внешняя сеть`.
2. Вставьте публичный URL Cloudflare/ngrok.
3. Вставьте токен доступа, если он задан в расширении.
4. Нажмите `Подключиться` или `Сохранить`/`Проверить`.

Если телефон уже подключен к ПК по локальной сети, кнопку `Запустить` в настройках APK можно использовать для запуска туннеля на ПК. Если телефон снаружи сети и еще не подключен, туннель сначала запускается в VS Code: `Remote Code on PC: Подключение` -> `Запустить внешний туннель`.

Для cloudflared пример ручной проверки:

```powershell
cloudflared tunnel --url http://127.0.0.1:8799 --no-autoupdate --protocol http2
```
...
code --install-extension .\remote-code-on-pc-1.1.49.vsix --force
```
...
curl.exe -L -I https://raw.githubusercontent.com/andruwko73/remote_code_on_pc/main/extension/remote-code-on-pc-1.1.49.vsix
```
...
Текущие версии:
- Android APK: 1.0.50, файл apk/app-debug.apk.
- VS Code extension: 1.1.49, файл extension/remote-code-on-pc-1.1.49.vsix.
...
Интерфейс Android:
- Экран подключения: переключатель локальная/внешняя сеть, IP ПК и порт для LAN, публичный URL Cloudflare/ngrok для внешней сети, токен, подключиться, отправить логи, очистить логи, обновить из GitHub, версия APK.
...
- Настройки должны быть компактными: статус, режим, провайдер Cloudflare/ngrok, APK, расширение, VS Code, IP, порт, public URL, запуск/остановка туннеля, токен, переподключение, обновление, логи.
