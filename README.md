...
- APK: `apk/app-debug.apk`, версия `1.0.62`.
- VSIX: `extension/remote-code-on-pc-1.1.67.vsix`, версия `1.1.67`.
- APK raw URL: https://raw.githubusercontent.com/andruwko73/remote_code_on_pc/main/apk/app-debug.apk
- VSIX raw URL: https://raw.githubusercontent.com/andruwko73/remote_code_on_pc/main/extension/remote-code-on-pc-1.1.67.vsix
- Порт по умолчанию: `8799`.

Последняя проверка: `2026-05-04`.

- Расширение: `npm run compile`, `npm test`, `npx vsce package --allow-missing-repository`, установка VSIX `1.1.67`; активное окно VS Code до reload еще может показывать старый `serverVersion`.
- API расширения: `/api/status`, `/api/codex/models`, `/api/codex/threads`, `/api/tunnel/status` отвечают `200`.
- Кнопки webview и APK: `npm test` проверяет `132` пункта, включая `data-action`, `approve/deny`, hover-кнопки сообщений, карточку изменений `undo/review/toggle`, `Ctrl+V`, `Enter`, `stop`, стартовые кнопки APK, меню текущего чата, вложения, голос и синхронизацию настроек composer.
...
- Внешняя сеть: APK теперь показывает отдельную ошибку, если публичный Keenetic/KeenDNS URL резолвится, но TCP-порт Remote Code недоступен. Для работы нужен прямой доступ/KeenDNS Direct и проброс TCP `8799 -> IP ПК:8799`; обычный облачный Keenetic URL без проброса произвольного TCP не заменяет этот маршрут.
...
```powershell
code --install-extension C:\Users\Admin\Documents\git\remote_code_on_pc\extension\remote-code-on-pc-1.1.67.vsix --force
```
После установки:
...
code --install-extension .\remote-code-on-pc-1.1.67.vsix --force
```
...
curl.exe -L -I https://raw.githubusercontent.com/andruwko73/remote_code_on_pc/main/extension/remote-code-on-pc-1.1.67.vsix
```
...
Текущие версии:
- Android APK: 1.0.62, файл apk/app-debug.apk.
- VS Code extension: 1.1.67, файл extension/remote-code-on-pc-1.1.67.vsix.
