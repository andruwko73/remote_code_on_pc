...
- Android APK: `1.0.123`.
- VS Code extension: `1.1.124`.
- Порт API по умолчанию: `8799`.
- Внешний доступ требует публичный URL и токен.
- Последняя проверка: `npm run compile`, `npm run bundle`, `node test-extension.js`, `scripts/run-e2e-smoke.ps1`, `scripts/capture-visual-regression.ps1`, `scripts/capture-visual-regression.ps1 -VsCodeFullscreen`, `:app:testDebugUnitTest`, `:app:assembleRelease`.
...
- Visual regression снимается скриптом `scripts/capture-visual-regression.ps1`; PNG сохраняются в `artifacts/screenshots/`, baseline хранится в `test-fixtures/visual-baseline/`. Для проверки чата “как в Codex” запускайте fullscreen-режим `scripts/capture-visual-regression.ps1 -VsCodeFullscreen`, он разворачивает VS Code и сравнивает с отдельным `vscode-fullscreen.png`.
