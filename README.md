...
- Android APK: `1.0.100`.
- VS Code extension: `1.1.97`.
- Порт API по умолчанию: `8799`.
- Внешний доступ требует публичный URL и токен.
- Последняя проверка: `npm run compile`, `node test-extension.js` (`194/194`), `:app:testDebugUnitTest`, `:app:assembleRelease`, установка и запуск APK `1.0.100` на эмуляторе.
...
- Код подключения поддерживается двумя способами: QR из меню подключения расширения или вставка строки `remote-code-pair:...`.
...
4. Выберите `Show Android QR code`, чтобы открыть QR и одновременно скопировать код подключения.
5. В приложении включите внешний режим и нажмите `Скан QR`; если камера недоступна, нажмите `Код` и вставьте скопированную строку.
