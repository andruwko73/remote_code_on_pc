...
- VSIX: `extension/remote-code-on-pc-1.1.70.vsix`, версия `1.1.70`.
- APK raw URL: https://raw.githubusercontent.com/andruwko73/remote_code_on_pc/main/apk/app-debug.apk
- VSIX raw URL: https://raw.githubusercontent.com/andruwko73/remote_code_on_pc/main/extension/remote-code-on-pc-1.1.70.vsix
- Порт по умолчанию: `8799`.

Последняя проверка: `2026-05-04`.

- Расширение: `npm run compile`, `npm test`, `npx vsce package --allow-missing-repository`, установка VSIX `1.1.70`; активное окно VS Code до reload еще может показывать старый `serverVersion`.
- API расширения: `/api/status`, `/api/codex/models`, `/api/codex/threads`, `/api/tunnel/status` отвечают `200`.
- Кнопки webview и APK: `npm test` проверяет `141` пункт, включая `data-action`, `approve/deny`, hover-кнопки сообщений, карточку изменений `undo/review/toggle`, `Ctrl+V`, `Enter`, `stop`, стартовые кнопки APK, явную кнопку/команду токена, меню копирования/создания нового токена, маскировку токена в APK, обязательный токен внешней сети, нормализацию публичного URL, WebSocket-защиту внешнего доступа, меню текущего чата, вложения, голос и синхронизацию настроек composer.
...
```powershell
code --install-extension C:\Users\Admin\Documents\git\remote_code_on_pc\extension\remote-code-on-pc-1.1.70.vsix --force
```
После установки:
...
code --install-extension .\remote-code-on-pc-1.1.70.vsix --force
```
...
curl.exe -L -I https://raw.githubusercontent.com/andruwko73/remote_code_on_pc/main/extension/remote-code-on-pc-1.1.70.vsix
```
...
- VS Code extension: 1.1.70, файл extension/remote-code-on-pc-1.1.70.vsix.
