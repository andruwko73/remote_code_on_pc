...
- APK: `apk/app-debug.apk`, версия `1.0.53`.
- VSIX: `extension/remote-code-on-pc-1.1.52.vsix`, версия `1.1.52`.
- APK raw URL: https://raw.githubusercontent.com/andruwko73/remote_code_on_pc/main/apk/app-debug.apk
- VSIX raw URL: https://raw.githubusercontent.com/andruwko73/remote_code_on_pc/main/extension/remote-code-on-pc-1.1.52.vsix
...
```powershell
code --install-extension C:\Users\Admin\Documents\git\remote_code_on_pc\extension\remote-code-on-pc-1.1.52.vsix --force
```
После установки:
...
Основной сценарий внешней сети теперь Keenetic/KeenDNS. Расширение не запускает `cloudflared` и не выдает одноразовый `trycloudflare.com` адрес. Вместо этого в расширении хранится постоянный публичный URL роутера:

```text
remoteCodeOnPC.publicUrl = https://name.keenetic.link:8799
```
...
2. Выберите `Задать публичный Keenetic URL`.
3. Вставьте URL вида `https://name.keenetic.link:8799`.
4. При необходимости создайте/скопируйте токен доступа.

`/api/tunnel/status` и `/api/status` отдают этот URL как `provider: keenetic`. `/api/tunnel/start` больше не запускает внешний процесс: если Keenetic URL задан, он сразу возвращает сохраненный адрес; если не задан - возвращает понятную ошибку.

В приложении:

1. На стартовом экране или в настройках включите `Внешняя сеть`.
2. Вставьте публичный Keenetic URL или, если телефон подключен локально, нажмите `Взять URL`, чтобы забрать сохраненный адрес из расширения.
...
```powershell
curl.exe -I https://name.keenetic.link:8799/api/status
```

В приложение нужно вставлять URL вида:

```text
https://name.keenetic.link:8799
```
...
code --install-extension .\remote-code-on-pc-1.1.52.vsix --force
```
...
curl.exe -L -I https://raw.githubusercontent.com/andruwko73/remote_code_on_pc/main/extension/remote-code-on-pc-1.1.52.vsix
```
...
- кнопка `Взять URL` возвращает сохраненный Keenetic URL или понятную ошибку;
...
Текущие версии:
- Android APK: 1.0.53, файл apk/app-debug.apk.
- VS Code extension: 1.1.52, файл extension/remote-code-on-pc-1.1.52.vsix.
