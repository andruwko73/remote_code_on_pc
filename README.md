...
- APK: `apk/app-debug.apk`, версия `1.0.63`.
...
- Кнопки webview и APK: `npm test` проверяет `137` пунктов, включая `data-action`, `approve/deny`, hover-кнопки сообщений, карточку изменений `undo/review/toggle`, `Ctrl+V`, `Enter`, `stop`, стартовые кнопки APK, явную кнопку/команду токена, обязательный токен внешней сети, WebSocket-защиту внешнего доступа, меню текущего чата, вложения, голос и синхронизацию настроек composer.
...
- Внешняя сеть: APK не подключается к публичному URL без токена доступа, а если Keenetic/KeenDNS URL резолвится, но TCP-порт Remote Code недоступен, показывает отдельную ошибку. Для работы нужен прямой доступ/KeenDNS Direct и проброс TCP `8799 -> IP ПК:8799`; обычный облачный Keenetic URL без проброса произвольного TCP не заменяет этот маршрут.
...
Текущие версии:
- Android APK: 1.0.63, файл apk/app-debug.apk.
