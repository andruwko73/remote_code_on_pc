...
- APK: `apk/app-debug.apk`, версия `1.0.56`.
- VSIX: `extension/remote-code-on-pc-1.1.60.vsix`, версия `1.1.60`.
- APK raw URL: https://raw.githubusercontent.com/andruwko73/remote_code_on_pc/main/apk/app-debug.apk
- VSIX raw URL: https://raw.githubusercontent.com/andruwko73/remote_code_on_pc/main/extension/remote-code-on-pc-1.1.60.vsix
- Порт по умолчанию: `8799`.

Последняя проверка: `2026-05-04`.

- Расширение: `npm run compile`, `npx vsce package --allow-missing-repository`, установка VSIX, `/api/status` с `serverVersion: 1.1.60`.
...
- публичные `/api/status` и `/api/tunnel/status` для диагностики подключения без раскрытия защищенных данных;
...
```powershell
code --install-extension C:\Users\Admin\Documents\git\remote_code_on_pc\extension\remote-code-on-pc-1.1.60.vsix --force
```
После установки:
...
5. При необходимости создайте/скопируйте токен доступа.

`/api/tunnel/status` и `/api/status` отдают этот URL как `provider: keenetic`. `/api/tunnel/start` больше не запускает внешний процесс: он возвращает сохраненный адрес, собирает URL из `remoteCodeOnPC.keeneticHost` или пытается определить KeenDNS через `my.keenetic.net`; если это невозможно, возвращает понятную ошибку.
...
3. Вставьте токен доступа, если он задан в расширении.
...
code --install-extension .\remote-code-on-pc-1.1.60.vsix --force
```
...
curl.exe -L -I https://raw.githubusercontent.com/andruwko73/remote_code_on_pc/main/extension/remote-code-on-pc-1.1.60.vsix
```
...
Текущие версии:
- Android APK: 1.0.56, файл apk/app-debug.apk.
- VS Code extension: 1.1.60, файл extension/remote-code-on-pc-1.1.60.vsix.
