...
- APK: `apk/app-debug.apk`, версия `1.0.60`.
- VSIX: `extension/remote-code-on-pc-1.1.66.vsix`, версия `1.1.66`.
- APK raw URL: https://raw.githubusercontent.com/andruwko73/remote_code_on_pc/main/apk/app-debug.apk
- VSIX raw URL: https://raw.githubusercontent.com/andruwko73/remote_code_on_pc/main/extension/remote-code-on-pc-1.1.66.vsix
- Порт по умолчанию: `8799`.

Последняя проверка: `2026-05-04`.

- Расширение: `npm run compile`, `npm test`, `npx vsce package --allow-missing-repository`, установка VSIX `1.1.66`; активное окно VS Code до reload еще может показывать старый `serverVersion`.
- API расширения: `/api/status`, `/api/codex/models`, `/api/codex/threads`, `/api/tunnel/status` отвечают `200`.
- Кнопки webview и APK: `npm test` проверяет `130` пунктов, включая `data-action`, `approve/deny`, hover-кнопки сообщений, карточку изменений `undo/review/toggle`, `Ctrl+V`, `Enter`, `stop`, стартовые кнопки APK, меню текущего чата, вложения, голос и синхронизацию настроек composer.
...
```powershell
code --install-extension C:\Users\Admin\Documents\git\remote_code_on_pc\extension\remote-code-on-pc-1.1.66.vsix --force
```
После установки:
...
code --install-extension .\remote-code-on-pc-1.1.66.vsix --force
```
...
curl.exe -L -I https://raw.githubusercontent.com/andruwko73/remote_code_on_pc/main/extension/remote-code-on-pc-1.1.66.vsix
```
...
Текущие версии:
- Android APK: 1.0.60, файл apk/app-debug.apk.
- VS Code extension: 1.1.66, файл extension/remote-code-on-pc-1.1.66.vsix.
