...
- VSIX: `extension/remote-code-on-pc-1.1.68.vsix`, версия `1.1.68`.
- APK raw URL: https://raw.githubusercontent.com/andruwko73/remote_code_on_pc/main/apk/app-debug.apk
- VSIX raw URL: https://raw.githubusercontent.com/andruwko73/remote_code_on_pc/main/extension/remote-code-on-pc-1.1.68.vsix
- Порт по умолчанию: `8799`.

Последняя проверка: `2026-05-04`.

- Расширение: `npm run compile`, `npm test`, `npx vsce package --allow-missing-repository`, установка VSIX `1.1.68`; активное окно VS Code до reload еще может показывать старый `serverVersion`.
- API расширения: `/api/status`, `/api/codex/models`, `/api/codex/threads`, `/api/tunnel/status` отвечают `200`.
- Кнопки webview и APK: `npm test` проверяет `134` пункта, включая `data-action`, `approve/deny`, hover-кнопки сообщений, карточку изменений `undo/review/toggle`, `Ctrl+V`, `Enter`, `stop`, стартовые кнопки APK, обязательный токен внешней сети, WebSocket-защиту внешнего доступа, меню текущего чата, вложения, голос и синхронизацию настроек composer.
...
```powershell
code --install-extension C:\Users\Admin\Documents\git\remote_code_on_pc\extension\remote-code-on-pc-1.1.68.vsix --force
```
После установки:
...
code --install-extension .\remote-code-on-pc-1.1.68.vsix --force
```
...
curl.exe -L -I https://raw.githubusercontent.com/andruwko73/remote_code_on_pc/main/extension/remote-code-on-pc-1.1.68.vsix
```
...
- VS Code extension: 1.1.68, файл extension/remote-code-on-pc-1.1.68.vsix.
