...
- Статистика change card в Android не обрезает общее число добавленных/удаленных строк, держит дельты в отдельной правой колонке, учитывает бинарные файлы в общем количестве и привязана к явному commit hash, чтобы старые сообщения не пересчитывались от текущего `HEAD`.
