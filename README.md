...
- Android APK: `1.0.103`.
- VS Code extension: `1.1.100`.
- Порт API по умолчанию: `8799`.
- Внешний доступ требует публичный URL и токен.
- Последняя проверка: `npm run compile`, `node test-extension.js`, `:app:testDebugUnitTest`, `:app:assembleRelease`, установка и запуск APK `1.0.103` на эмуляторе.
...
- Статистика change card в Android не обрезает общее число добавленных/удаленных строк, учитывает бинарные файлы в общем количестве и привязана к явному commit hash, чтобы старые сообщения не пересчитывались от текущего `HEAD`.
