...
- Android APK: `1.0.120`.
- VS Code extension: `1.1.121`.
- Порт API по умолчанию: `8799`.
- Внешний доступ требует публичный URL и токен.
- Последняя проверка: `npm run compile`, `npm run bundle`, `node test-extension.js`, `:app:testDebugUnitTest`, `:app:assembleRelease`.
...
- Visual regression снимается скриптом `scripts/capture-visual-regression.ps1`; PNG сохраняются в `artifacts/screenshots/`.
