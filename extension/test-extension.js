...
assert(androidBuildGradle.includes('versionCode = 80') && androidBuildGradle.includes('versionName = "1.0.79"') && androidBuildGradle.includes('signingConfig = signingConfigs.getByName("debug")'), 'Android release artifact can update existing sideload installs', 'release APK should be version-bumped and signed for sideload updates');
assert(mainActivity.includes('UpdateReadyDialog') && mainActivity.includes('PendingVerifiedApk') && mainActivity.includes('startActivityForResult(intent, 12078)') && !mainActivity.includes('Intent.FLAG_ACTIVITY_NEW_TASK'), 'Android updater waits for a user install tap', 'verified APK should open the package installer from a foreground user action');
