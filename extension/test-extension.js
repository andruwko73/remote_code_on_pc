...
assert(androidBuildGradle.includes('versionCode = 83') && androidBuildGradle.includes('versionName = "1.0.82"') && androidBuildGradle.includes('signingConfig = signingConfigs.getByName("debug")'), 'Android release artifact can update existing sideload installs', 'release APK should be version-bumped and signed for sideload updates');
assert(mainActivity.includes('UpdateReadyDialog') && mainActivity.includes('PendingVerifiedApk') && mainActivity.includes('pendingVerifiedApk = null') && mainActivity.includes('Handler(Looper.getMainLooper()).post') && mainActivity.includes('Intent.ACTION_INSTALL_PACKAGE') && mainActivity.includes('startActivityForResult(intent, updateInstallRequestCode)') && !mainActivity.includes('Intent.FLAG_ACTIVITY_NEW_TASK'), 'Android updater waits for a user install tap and closes its dialog before handoff', 'verified APK should dismiss the app dialog and open the package installer from a foreground user action');
