...
assert(androidBuildGradle.includes('versionCode = 84') && androidBuildGradle.includes('versionName = "1.0.83"') && androidBuildGradle.includes('signingConfig = signingConfigs.getByName("debug")'), 'Android release artifact can update existing sideload installs', 'release APK should be version-bumped and signed for sideload updates');
assert(mainActivity.includes('UpdateReadyDialog') && mainActivity.includes('UpdateStatusDialog') && mainActivity.includes('onStatus("Скачивание обновления') && mainActivity.includes('onStatus("Проверка APK') && mainActivity.includes('PendingVerifiedApk') && mainActivity.includes('pendingVerifiedApk = null') && mainActivity.includes('Handler(Looper.getMainLooper()).post') && mainActivity.includes('Intent.ACTION_INSTALL_PACKAGE') && mainActivity.includes('startActivityForResult(intent, updateInstallRequestCode)') && !mainActivity.includes('Intent.FLAG_ACTIVITY_NEW_TASK'), 'Android updater shows states, waits for a user install tap, and closes its dialog before handoff', 'verified APK should show download/check/install states, dismiss the app dialog, and open the package installer from a foreground user action');
...
assert(codexScreen.includes('BasicTextField') && codexScreen.includes('heightIn(min = 42.dp, max = 108.dp)') && codexScreen.includes('Modifier.size(38.dp)'), 'Android composer is compact like Codex', 'mobile composer should avoid excessive vertical height');
