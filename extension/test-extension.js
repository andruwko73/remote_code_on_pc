...
assert(androidBuildGradle.includes('versionCode = 88') && androidBuildGradle.includes('versionName = "1.0.87"') && androidBuildGradle.includes('signingConfig = signingConfigs.getByName("debug")'), 'Android release artifact can update existing sideload installs', 'release APK should be version-bumped and signed for sideload updates');
assert(mainActivity.includes('UpdateReadyDialog') && mainActivity.includes('UpdateStatusDialog') && mainActivity.includes('onStatus("Скачивание обновления') && mainActivity.includes('onStatus("Проверка APK') && mainActivity.includes('PendingVerifiedApk') && mainActivity.includes('onReadyDialogFinished = { pendingVerifiedApk = null }') && mainActivity.includes('onInstallPermissionRequired = { pendingVerifiedApk = update }') && mainActivity.includes('Handler(Looper.getMainLooper()).post') && mainActivity.includes('Intent.ACTION_INSTALL_PACKAGE') && mainActivity.includes('startActivity(intent)') && mainActivity.includes('startActivity(viewIntent)') && !mainActivity.includes('Intent.EXTRA_RETURN_RESULT') && !mainActivity.includes('startActivityForResult') && !mainActivity.includes('Intent.FLAG_ACTIVITY_NEW_TASK'), 'Android updater shows states, waits for a user install tap, keeps retry alive, and lets Package Installer show Open/Done', 'verified APK should use normal foreground installer handoff without return-result mode');
